...
def parse(src: str):
    ...
    desc: list[list[str]] = []           # one inner list of lines per paragraph

    def flush_proj():
        nonlocal cur_proj
        if cur_proj is not None:
            paras = [" ".join(lines).strip() for lines in desc]
            cur_proj["desc"] = [p for p in paras if p]
    ...
    for raw in src.splitlines():
        s = raw.strip()
        if not s:
            # A blank line inside a description body ends the paragraph. Everywhere
            # else -- between fields, between cards -- it stays insignificant.
            if cur_proj is not None and desc and desc[-1]:
                desc.append([])
            continue
        if s.startswith("# "):
            continue
...
def render_card(p: dict, order: list[str]) -> str:
    ...
    # Only a write-up page claims the title. A repo is reachable via the "Source"
    # link in the meta row, so linking the title there too was both redundant and
    # a misleading affordance -- a title link should lead to a fuller description.
    if p["page"]:
...
CSS = """\
    ...
    .wrap {
      /* Rail geometry lives here so the jump-nav column and anything that has to
         line up with the card column (the filter banner) cannot drift apart. Both
         are in rem, not em: em would resolve against each consumer's own
         font-size, and .filter-status sets its own. */
      --rail: 11rem;
      --rail-gap: 2.5rem;
      max-width: 60em;
      margin: 0 auto;
    ...
    /* Active-filter banner: the only filter cue on mobile, where the chips hide.
       Indented past the jump-nav rail so it starts at the card column, not at
       "Contents". */
    .filter-status {
      display: none;
      align-items: center;
    ...
    .toc {
      ...
      /* Flex items default to min-width:auto, which refuses to go narrower than
         ...
         which word was longest, resizing the rail (and with it the card column)
